...
static std::vector<std::string> extractCompileFlags(const fs::path& dbPath) {
  ...
  std::vector<std::string> seenIsystem;
  ...
  for (const auto& entry : db) {
    ...
    for (size_t j = 0; j < args.size(); ++j) {
      ...
      if (a.size() > 2 && a.substr(0, 2) == "-I") {
        addOnce(a);
      } else if (a == "-I" && j + 1 < args.size()) {
        addOnce("-I" + args[++j]);
      } else if (a == "-isystem" && j + 1 < args.size()) {
        std::string path = args[++j];
        if (std::find(seenIsystem.begin(), seenIsystem.end(), path) == seenIsystem.end()) {
          seenIsystem.push_back(path);
...
int main(int argc, char** argv) {
  ...
  for (const auto& header : cfg.headers) {
    ...
    std::cerr << "[forge-reflect] Clang args:\n";
